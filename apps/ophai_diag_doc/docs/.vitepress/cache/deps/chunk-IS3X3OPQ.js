...
var ErrorTypeStrings$1 = {
  ...
  [11]: "app warnHandler",
  [12]: "ref function",
  [13]: "async components loader",
  [14]: "scheduler flush",
  [15]: "components update",
...
var devtoolsComponentRemoved = (component) => {
  if (devtools$1 && typeof devtools$1.cleanupBuffer === "function" && // remove the components if it wasn't buffered
  ...
  if (children.length > 1) {
    ...
    for (const c of children) {
      if (c.type !== Comment) {
        if (hasFound) {
          warn$1(
            "<transition> can only be used on a single element or components. Use <transition-group> for lists."
          );
          break;
        }
        child = c;
...
var PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    } else if (currentRenderingInstance && (!isString(key) || // #1091 avoid internal isRef/isVNode checks on components instance leading
  ...
  const patchBlockChildren = (oldChildren, newChildren, fallbackContainer, parentComponent, parentSuspense, namespace, slotScopeIds) => {
    for (let i = 0; i < newChildren.length; i++) {
      ...
      const container = (
        ...
        !isSameVNodeType(oldVNode, newVNode) || // - In the case of a components, it could contain anything.
...
var isBuiltInTag = makeMap("slot,components");
function validateComponentName(name, { isNativeTag }) {
  if (isBuiltInTag(name) || isNativeTag(name)) {
    warn$1(
      "Do not use built-in or reserved HTML elements as components id: " + name
    );
  }
}
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-IS3X3OPQ.js.map
